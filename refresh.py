...
NETUID   = 118
TARGETS  = ()             # no pinned comps for SN118
NEIGHBORS = 3             # ranks above/below SN118 to show

# Manual labels: coldkey -> (label, highlight, is_validator). highlight: green|purple|gold|orange
MANUAL_LABELS = {
    "5DUJfND9r2BjiJHNKSxJxzT7ptrxiseFe1GxeMwcTov52Y55": ("SN118 owner", "green", False),
...
def main():
    ...
    seen_existed = os.path.exists("holders_seen.json")
    seen = _load("holders_seen.json")             # {coldkey: {"date": "YYYY-MM-DD", "baseline": bool}}
    for h in holders:
        ck = h["coldkey"]
        if ck in entry and entry[ck].get("entered"):
            ...
            rec = seen.get(ck)
            if rec is None:
                # first run creates the file -> everyone is a baseline holder (unknown entry date);
                # wallets first detected on later runs are genuinely new arrivals.
                rec = {"date": today, "baseline": not seen_existed}; seen[ck] = rec
            elif isinstance(rec, str):
                rec = {"date": rec, "baseline": False}; seen[ck] = rec
            if rec.get("baseline"):
                h["entered"] = None; h["entered_method"] = "baseline"
            else:
                h["entered"] = rec["date"]; h["entered_method"] = "first-seen"
        try:
            if h.get("entered"):
                days = (datetime.now(timezone.utc) - datetime.fromisoformat(h["entered"] + "T00:00:00+00:00")).days
                h["is_new"] = days <= 45 and h["entered_method"] != "trades"
            else:
                h["is_new"] = False
        except Exception:
            h["is_new"] = False
